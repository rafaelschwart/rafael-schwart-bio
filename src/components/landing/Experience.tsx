...
import { experience, pipelineStages, type Experience as Exp } from "./data"
import { Chip, CornerTick, Eyebrow } from "./atoms"
import { snapSwap } from "./motion"

/**
 * Experience view — nine-company selector on paper with a dark detail panel
 * on ink. Desktop: selector rail + side panel. Mobile (≤860px): accordion —
 * the detail opens inline under the tapped company, so nothing scrolls
 * sideways. The featured Motorola card carries the live NPI program pipeline
 * (CONCEPT → PILOT → RAMP → VOLUME) with the ramp node pulsing.
 */

function ExpDetail({ e }: { e: Exp }) {
  return (
    <>
...
export function Experience() {
  ...
  const select = (i: number, btn: HTMLButtonElement) => {
    setSel(i)
    snapSwap(panelRef.current)
    // Accordion above may collapse and shift the layout — keep the tapped
    // company row in view.
              ...
              return (
                <Fragment key={e.company}>
                  <button
                    ...
                    onClick={(ev) => select(i, ev.currentTarget)}
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                      ...
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 9 }}>
                        ...
                        {/* Mobile-only accordion caret (CSS shows it ≤860px) */}
                        <span data-exp-caret aria-hidden="true">
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={active ? "var(--signal)" : "var(--mute)"} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
                            <path d="m6 9 6 6 6-6" />
                          </svg>
                        </span>
                      </span>
                  ...
                  {/* Mobile accordion body — hidden on desktop via CSS */}
                  {active && (
                    <div
                      key={e.company}
                      data-exp-inline
